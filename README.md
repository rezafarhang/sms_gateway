...
**Problem**: When Redis fails, all workers fall back to direct DB updates → potential DB overload.
...
- If > 50% fail in 30s window → OPEN circuit (reject new fallbacks)
- After 60s timeout → HALF_OPEN (allow 10 test requests)
- If tests succeed → CLOSED (resume normal)
...
- Example: Query for "today 09:00-11:00" → 12 buckets (09:00-09:10, 09:10-09:20, ..., 10:50-11:00)
...
Buckets:
- 12:00-12:10 → Cache hit (immutable)
- 12:10-12:20 → Cache hit (immutable)
- ...
- 14:20-14:30 → Cache hit (immutable)
- 14:30-14:35 → DB query (current bucket, mutable)
